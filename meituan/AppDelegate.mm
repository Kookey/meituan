...
#define BAIDU_AK @"PEzDpCea07uzWjkuOGbBy0seTcOjavGj"
    ...
    [[NetworkSingleton sharedManager] getAdvLoadingImage:nil url:urlStr successBlock:^(id responseBody){
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            if (dataArray.count>0) {
                ...
                NSString *filePath = [[paths firstObject] stringByAppendingPathComponent:@"loading.png"];   // 保存文件的名称
...
-(void)initAdvView{
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask, YES);
    NSString *filePath = [[paths firstObject] stringByAppendingPathComponent:@"loading.png"];
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    BOOL result = [_mapManager start:BAIDU_AK generalDelegate:nil];
    NSLog(@"%@", result?@"成功":@"失败");
    [self initRootVC];
    [self initAdvView];
