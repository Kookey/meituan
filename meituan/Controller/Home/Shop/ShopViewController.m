...
-(void)setNav{
  ...
  [self.view addSubview:backView];
    //下划线
  UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, NAVIGATIONBAR_HEIGHT - 0.5, screen_width, 0.5)];
...
-(void)getShopRecommendData{
  ...
  [[NetworkSingleton sharedManager] getShopRecommendResult:nil url:urlStr successBlock:^(id responseBody){
    ...
    if (dealsArray.count > 0) {
      NSArray *arr = [ShopRecommendModel mj_objectArrayWithKeyValuesArray:dealsArray];
      [_shopRecommendArray addObjectsFromArray:arr];
    }
    weakself.tableView.hidden = NO;
    [_activityView stopAnimating];
    [weakself.tableView reloadData];
    ...
    if (indexPath.row == 0) {
      ...
      if (cell == nil) {
        ...
        [cell.contentView addSubview:tuiBtn];
          //已售
        UILabel *soldedLabel = [[UILabel alloc] initWithFrame:CGRectMake([tuiBtn wxl_r]+10, 5, 100, 30)];
      ...
      if (_shopInfoM && _shopInfoM.solds) {
